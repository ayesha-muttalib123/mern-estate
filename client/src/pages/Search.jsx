...
function Search() {
  ...
  useEffect(() => {
    const fetchListings = async () => {
      ...
      try {
        const response = await fetch(`/api/listing/get?${new URLSearchParams(searchParams)}`);
  ...
  return (
    <div className="p-5 flex flex-col md:flex-row md:h-screen">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 border-b-2 md:border-r-2 md:min-h-screen p-5">
      ...
      <div className="flex-grow  p-4">
        ...
        {loading && (
          <p className="text-center text-slate-700 p-3 font-semibold">Loading...</p>
        )}
